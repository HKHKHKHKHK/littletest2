...
class ProxyMiddleWare(object):
    ...
    def process_request(self, request, spider):
        if not 'proxy' in request.meta or self.isblacked:
            self.get_random_proxy()
        print('current IP is ', self.current_proxy)
    ...
    #     # 如果返回的response状态不是200，重新生成当前request对象
        if response.status in self.RETRY_HTTP_CODES  or 'captcha' in response.url:
            print(self.current_proxy,'is being banned')
            if not self.isblacked:
                self.isblacked = True
            self.get_random_proxy()
    ...
    #     if not self.current_proxy or self.current_proxy in self.banlist:
    #         self.lock.acquire()
    #         response = requests.get(self.proxy_url)
    #         if response.status_code == 200:
    #             self.current_proxy = 'https://' + response.text
